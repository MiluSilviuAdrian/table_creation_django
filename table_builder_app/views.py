...
def update_table(request, pk):
    ...
    if not data:
        return Response(
            {"error": "No fields provided."}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    return Response(
        {"success": "Table updated successfully."}, status=status.HTTP_200_OK
    )
...
def create_table_row(request, pk):
    ...
    if extra_fields:
        return Response(
            {"error": f"Extra fields not allowed: {', '.join(extra_fields)}"},
    ...
    for field in table.fields:
        ...
        if not isinstance(field_value, expected_field_type):
            return Response(
                {
                    "error": f"Invalid value '{field_value}' for field '{field_name}'. Expected type: {expected_field_type.__name__}."
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
